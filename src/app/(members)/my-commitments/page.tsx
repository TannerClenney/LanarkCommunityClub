    ...
    <div className="space-y-6">
      ...
      {groupedTasks.length === 0 ? (
        <div className="rounded-xl border border-dashed border-stone-200 bg-white p-8 text-center shadow-sm">
          <p className="text-base text-zinc-600">You do not have any active commitments yet.</p>
          <Link href="/calendar" className="mt-3 inline-block text-base font-medium text-emerald-700 hover:underline">
        ...
        <div className="space-y-6">
          {groupedTasks.map((eventGroup) => (
            <section key={eventGroup.eventSlug} className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div>
                  <h2 className="text-lg font-bold text-zinc-900">{eventGroup.eventTitle}</h2>
                  <p className="text-base text-zinc-600">{eventGroup.areas.reduce((count, area) => count + area.tasks.length, 0)} commitment(s)</p>
                </div>
                <Link href={`/members/events/${eventGroup.eventSlug}`} className="text-base font-medium text-emerald-700 hover:underline">
                  View event →
                </Link>
              </div>
              ...
              <div className="space-y-6">
                {eventGroup.areas.map((area) => (
                  <div key={`${eventGroup.eventSlug}-${area.areaSlug}`} className="rounded-lg border border-stone-200 bg-stone-50 p-5">
                    <div className="mb-3 flex items-center justify-between gap-3">
                      <div>
                        <h3 className="text-base font-semibold text-zinc-900">{area.areaName}</h3>
                        <p className="text-sm text-zinc-600">Owned tasks in this area</p>
                      </div>
                      <Link
